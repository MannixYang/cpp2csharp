...
public:
    StringObj() = default;
    void set(std::string ss){s = ss; std::cout << "c++ StringObj set: " << s << std::endl;}
...
static std::string utf16_to_utf8(const wchar_t *str) {
  ...
  WideCharToMultiByte(
      CP_UTF8, 0, str, -1, &utf8_str[0], utf8_size, nullptr, nullptr);

  return utf8_str;
}

static std::wstring utf8_to_utf16(const char *str) {
  const auto utf16_size =
      MultiByteToWideChar(CP_UTF8, 0, str, -1, nullptr, 0);
...
__declspec(dllexport) void string_obj_set(C_StringObj self, const String ss){
    reinterpret_cast<StringObj*>(self.ptr)->set(*reinterpret_cast<const std::basic_string<char>*>(ss.ptr));
}

__declspec(dllexport) String string_obj_get(C_StringObj self){
    return {new std::basic_string<char>(reinterpret_cast<StringObj*>(self.ptr)->get()), true};
}
...
__declspec(dllexport) void RegisterStringCallback(CSharpStringHelperCallback callback) {
  csharp_string_callback = callback;
}

// C ABI wrapper
__declspec(dllexport) void * csharp_new_string_obj(){
    return  new_string_obj().ptr;
}

__declspec(dllexport) void csharp_string_obj_set(void* self_ptr, bool self_mem_owner, const char * jarg1){
    std::string utf8_jarg1 = utf16_to_utf8(reinterpret_cast<const wchar_t *>(jarg1));
    C_StringObj self = {self_ptr, self_mem_owner};
    string_obj_set(self, new_String_1(utf8_jarg1.c_str()));
}

__declspec(dllexport) const char * csharp_string_obj_get(void* self_ptr, bool self_mem_owner){
    C_StringObj self = {self_ptr, self_mem_owner};
